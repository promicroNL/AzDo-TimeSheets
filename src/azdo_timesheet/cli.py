...
REMAINING_WORK_STRATEGIES = (
    "none",
    "decrement",
    "recalc_from_original",
    "interactive",
    "prompt",
)
...
def load_config(path: Path) -> Config:
    ...
    remaining_work_strategy = normalize_remaining_strategy(
        data.get("remaining_work_strategy", "none")
    )
    return Config(
        ...
        remaining_work_strategy=remaining_work_strategy,
...
def save_config(path: Path, config: Config) -> None:
    payload = {
        ...
        "remaining_work_strategy": normalize_remaining_strategy(
            config.remaining_work_strategy
        ),
    ...
    for item in work_items_response.get("value", []):
        ...
        records.append(
            (
                ...
                fields_data.get(FIELD_ORIGINAL_ESTIMATE),
                fields_data.get(FIELD_REMAINING_WORK),
                fields_data.get(FIELD_COMPLETED_WORK),
                now,
            )
        )
    connection.executemany(
...
def resolve_remaining_after(
    *,
    ...
    strategy = normalize_remaining_strategy(strategy)
    ...
    if strategy == "interactive":
        if not interactive_allowed or not sys.stdin.isatty():
            return remaining_before
...
def plan_deltas(
    ...
    remaining_work_strategy: str,
    *,
    work_items: dict[int, WorkItem | None] | None = None,
    ...
    for work_item_id, group in grouped.items():
        ...
        work_item = (
            work_items.get(work_item_id)
            if work_items is not None
            else fetch_work_item(connection, work_item_id)
        )
        ...
        remaining_after = resolve_remaining_after(
...
def fetch_work_item_remote(
    *, config: Config, work_item_id: int
) -> WorkItemRemote:
    fields = ",".join(
        [
            "System.Title",
    ...
    now = datetime.utcnow().isoformat()
    return WorkItemRemote(
        work_item=WorkItem(
            work_item_id=response.get("id"),
            title=fields_data.get("System.Title"),
            state=fields_data.get("System.State"),
            original_estimate=fields_data.get(FIELD_ORIGINAL_ESTIMATE),
            remaining_work=fields_data.get(FIELD_REMAINING_WORK),
            completed_work=fields_data.get(FIELD_COMPLETED_WORK),
            updated_at=now,
        ),
        fields_present=set(fields_data.keys()),
    )
...
def build_patch_operations(
    *,
    ...
    completed_op = "replace" if FIELD_COMPLETED_WORK in fields_present else "add"
    operations.append(
        {
            "op": completed_op,
            "path": f"/fields/{FIELD_COMPLETED_WORK}",
            "value": completed_after,
        }
    )
    normalized_strategy = normalize_remaining_strategy(remaining_strategy)
    if normalized_strategy != "none" and remaining_after is not None:
        remaining_op = "replace" if FIELD_REMAINING_WORK in fields_present else "add"
        operations.append(
            {
                "op": remaining_op,
                "path": f"/fields/{FIELD_REMAINING_WORK}",
                "value": remaining_after,
            }
        )
    return operations
...
def sync_command(args: argparse.Namespace) -> int:
    ...
    with connect_db(config.storage_path) as connection:
        try:
            ...
            work_items: dict[int, WorkItem | None] = {}
            work_items_remote: dict[int, WorkItemRemote] = {}
            if args.apply:
                for work_item_id in work_item_ids:
                    remote = fetch_work_item_remote(
                        config=config,
                        work_item_id=work_item_id,
                    )
                    work_items_remote[work_item_id] = remote
                    work_items[work_item_id] = remote.work_item
            else:
                for work_item_id in work_item_ids:
                    work_items[work_item_id] = fetch_work_item(
                        connection,
                        work_item_id,
                    )
            deltas = plan_deltas(
                ...
                interactive_allowed=not args.apply,
            )

            for delta in deltas:
                delta_strategy = normalize_remaining_strategy(delta.remaining_strategy)
            ...
            if args.apply:
                now = datetime.utcnow().isoformat()
                for delta in deltas:
                    remote = work_items_remote.get(delta.work_item_id)
                    if not remote:
                        raise ValueError(
                            "Missing Azure DevOps snapshot for work item "
                            f"{delta.work_item_id}."
                        )
                    work_item = remote.work_item
                    if is_closed_state(work_item.state) and not config.allow_sync_closed_items:
                        raise ValueError(
                            f"Work item {delta.work_item_id} is closed. "
                            "Enable allow_sync_closed_items to sync."
                        )
                    remaining_after = resolve_remaining_after(
                        strategy=remaining_work_strategy,
                        remaining_before=work_item.remaining_work,
                        original_estimate=work_item.original_estimate,
                        completed_after=delta.completed_after,
                        hours_logged=delta.total_hours,
                    ...
                    operations = build_patch_operations(
                        ...
                        remaining_after=remaining_after,
                        remaining_strategy=remaining_work_strategy,
                        fields_present=remote.fields_present,
                    )
                    response = azdo_request(
                        config=config,
                        method="PATCH",
                        path=(
                            f"_apis/wit/workitems/{delta.work_item_id}?api-version=7.0"
                        ),
                        payload=operations,
                        content_type="application/json-patch+json",
                    )
                    response_fields = response.get("fields", {})
                    updated_completed = response_fields.get(FIELD_COMPLETED_WORK)
                    updated_remaining = response_fields.get(FIELD_REMAINING_WORK)
                    updated_original = response_fields.get(FIELD_ORIGINAL_ESTIMATE)
                    updated_state = response_fields.get("System.State")
                    updated_title = response_fields.get("System.Title")
                    connection.execute(
                        """
                        INSERT INTO work_items (
                            work_item_id, title, state, original_estimate, remaining_work,
                ...
                connection.commit()
                print("Synced entries to Azure DevOps and marked locally.")
            else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init_parser.add_argument(
        ...
        default="none",
        choices=REMAINING_WORK_STRATEGIES,
        help="Remaining Work strategy (default: none; interactive prompts only in dry-run)",
    )
    init_parser.add_argument(
        "--wiql",
        dest="wiql_query",
